...
if (typeof window !== "undefined") {
  ...
  function ppCardHTML(q) {
    ...
    h += st.rows.map(function (r) {
      ...
      return '<div class="li" style="padding:7px 0;align-items:center"><div class="grow"><div class="nm" style="font-size:14px">' + r.n + ' · ' + ppMoney(r.cents) + ' ' + ppBadge(r.state) + '</div><div class="sub">due ' + ppE((typeof fmtDate === "function") ? fmtDate(r.due) : r.due) + (r.paidAt ? ' · paid ' + ppE(new Date(r.paidAt).toLocaleDateString()) + (r.method ? ' by ' + ppE(r.method) : '') : r.sentAt ? ' · sent ' + ppE(new Date(r.sentAt).toLocaleDateString()) : ' · not sent yet') + (r.link ? ' · <a href="' + ppE(r.link) + '" target="_blank" rel="noopener">link</a>' : '') + '</div></div><div style="display:flex;gap:6px;flex:0 0 auto">' + act + '</div></div>';
    }).join("");
