...
function tcClockHTML() {
  ...
  if (recent.length) {
    h += `<div class="secthd"><h2>Your recent shifts</h2><span class="ct">${recent.length}</span></div><div class="card">` +
      recent.map(e => `<div class="li" style="cursor:pointer" onclick="tcEditPunch('${e.id}')"><div class="grow"><div class="nm" style="font-size:14px">${esc(tcEntryLabel(e))} <span class="sub" style="font-weight:400">✎ tap to fix times</span></div>
        <div class="sub">${fmtDate(new Date(e.clockIn).toISOString().slice(0,10))} · ${tcFmtDur(e.clockOut - e.clockIn)} · ${tcMiles(e)} mi${e.milesConfirmed ? "" : " est"}${e.riderRole === "passenger" ? " · 🧍 passenger" : (e.vehicle ? " · 🚚 " + esc(e.vehicle) + (e.trailerId ? " + 🚛" : "") : "")}${e.milesFlag ? " · ⚠" : ""}${tcSaneMiles(e) ? " · ⚠ review" : ""}${e.odoPending ? " · 🕑 odo" : ""}</div>${(typeof shiftPunchDesc==="function"&&shiftPunchDesc(e.id))?`<div class="sub" style="white-space:normal;margin-top:3px;color:var(--ink)">📝 ${esc(shiftPunchDesc(e.id))}</div>`:""}${e.odoPending && (typeof tcCanEditEntry === "function" && tcCanEditEntry(e)) ? `<div class="sub" style="margin-top:2px"><a href="#" onclick="event.stopPropagation();tcAddEndOdo('${e.id}');return false" style="color:var(--brand-text);font-weight:700">＋ Add ending odometer</a></div>` : ""}</div>${tcSourceBadge(e)}</div>`).join("") + `</div>`;
  }
  return h;
}
