...
window.wizDelete=function(){if(!WZ.id){exitWizard();return;}if(wizLockedAlert())return;if(!confirm("Delete this quote? It goes to the Archive for 60 days — you can restore it there."))return;const q=D().quotes.find(x=>x.id===WZ.id);if(q){q.deleted=true;q.deletedAt=now();touch(q);if(typeof logChange==="function")logChange("delete","quote",q.id,"Deleted quote"+(q.cust?" · "+q.cust:""));save();}wizClearDraft();exitWizard();};
