...
const WZ_FIELDS={
 ...
 housewatch:[{k:"size",t:"sel",label:"Home size",opts:[["small","Up to 4 bedrooms"],["medium","5–8 bedrooms"],["large","9+ / estate"]]},{k:"freq",t:"sel",label:"Frequency",opts:[["monthly","Monthly"],["bi-weekly","Bi-weekly"],["weekly","Weekly"]]},{k:"access",t:"sel",label:"Area",opts:[["","Standard: Moyock–Duck, Manteo–Nags Head"],["north","North of Duck (Corolla)"],["south","South of Nags Head (Hatteras Island)"],["carova","Carova"],["ocracoke","Ocracoke"]]},{k:"miles",t:"num",label:"Miles from Harbinger, one-way",ph:"only for north / south"}],
...
function guessBandKey(name){ name=String(name||"").toLowerCase(); if(/paver/.test(name))return "paver"; if(/stepping.?stone|stepping stone|stone path|rock path/.test(name))return "steppath"; if(/french ?drain|trench drain/.test(name))return "frenchdrain"; if(/downspout|gutter repair|gutter.*(repair|reroute|relocat)/.test(name))return "downspout"; if(/handyman/.test(name))return "handyman"; if(/hot tub/.test(name))return "hottubdemo"; if(/paver.*(removal|demo)/.test(name))return "paverdemo"; if(/deck removal|deck demo/.test(name))return "deckdemo"; if(/fence removal|fence demo/.test(name))return "fencedemo"; if(/strip.?out|interior demo|gut/.test(name))return "intdemo"; if(/concrete (slab|wall) removal/.test(name))return "concdemo"; if(/demo|demolition/.test(name))return "demo"; if(/brush|shrub|stump|tree/.test(name))return "brush"; if(/junk|clean.?out|move.?out|haul/.test(name))return "junk"; return null; }
