...
function rcptTableHTML(rows, dups) {
  ...
  // Cap reread checkboxes: a column only when there ARE review rows here to select (js/88 owns the cells).
  const showChk = (typeof capRcptRowCheckbox === "function") && rcptFinFull() && rows.some(r => r && r.store === "review" && r.receiptId);
  const chkTh = showChk ? `<th style="padding:8px 4px;border-bottom:2px solid var(--line)" title="Select rows to reread"></th>` : "";
  let h = `<div class="card" style="padding:4px 4px 6px;overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px">
    <thead><tr>${chkTh}${th("date", "Date")}${th("vendor", "Vendor")}${th("amount", "Amount", "right")}${th("type", "Type")}${th("category", "Category")}${th("job", "Job / Customer")}${th("uploader", "By")}${th("attributedTo", "For")}${th("card", "💳 Card")}${th("order", "Ref #")}<th style="padding:8px 6px;border-bottom:2px solid var(--line)">📎</th>${th("status", "Status")}</tr></thead><tbody>`;
  ...
  rows.slice(0, 500).forEach(r => {
    ...
    // 🔀 Split — surfaced on the ROW (not only in the edit modal) so splitting a receipt into parts is
    // discoverable from the table; opens the editor and jumps straight into the split allocator.
