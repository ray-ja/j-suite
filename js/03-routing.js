...
function render(){
  ...
  (({today:rToday,accounts:rAccounts,quotes:rQuotes,pipeline:rPipeline,schedule:rSchedule,messages:rMessages,map:rMap,sales:rSales,todo:rTodos,plan:rPlan,training:rTraining,market:rMarket,opps:rOpps,sites:rSites,buildplan:rBuildPlan,inventory:rInventory,resale:rResale,time:rTime,finance:rFinance,receipts:rReceipts,data:rData,approvals:rApprovals,admin:rAdmin,playbook:rPlaybook,booking:rBooking}[TAB])||rToday)();
...
// restriction (null = "all"). Keeps a new niche tool (e.g. escape-room Booking) out of obx/jam.
const ORG_OPTIN_TABS = ["booking"];
const ORG_TEMPLATES = {
  full: null,                                                                 // field services (OBX / Jamieson) — every tool
  bookings: ["booking","messages","schedule","accounts","finance","receipts","time","playbook"],   // e.g. an escape room — booking page + customers, calendar, money, comms
