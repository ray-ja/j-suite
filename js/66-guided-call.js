...
function gcBody() {
  ...
  h += gcStep("2b", "Curb or crew? (junk calls)", `"Is it stuff one person can carry, and can it be at the curb or the garage door? If so it's our one-person rate, from $${(typeof JUNK_MIN_BY_CREW!=="undefined")?JUNK_MIN_BY_CREW[1]:175}. Inside the house, upstairs, or heavy pieces, that's the two-person crew, from $${(typeof JUNK_MIN_BY_CREW!=="undefined")?JUNK_MIN_BY_CREW[2]:300}."  → Quote BOTH; let them pick. Curbside solo = Ray's Tue/Thu slot without pulling Chase.`, `<label class="toggle" style="margin:0"><input type="checkbox" ${GC.solo ? "checked" : ""} onchange="GC.solo=this.checked"><span style="margin:0">👤 One-person / curbside job</span></label>`);
  h += gcStep("3", "Where & when", `"Got it. What's the property address? … And when were you hoping to have it done?"  → The address lets you price + map it; urgency means you can hold your price.`, `<div class="acwrap"><input id="gc_addr" placeholder="Start typing the address…" value="${esc(GC.address || "")}" oninput="GC.address=this.value;addrSuggest('gc_addr','gc_abox')" onchange="GC.address=this.value" autocomplete="off"><div class="acbox" id="gc_abox"></div></div><input id="gc_unit" placeholder="Unit / apt / suite (line 2, optional)" value="${esc(GC.unit || "")}" oninput="GC.unit=this.value" autocomplete="off" style="margin-top:6px"><input id="gc_when" placeholder="Timeline / urgency (ASAP, this month…)" value="${esc(GC.timeline || "")}" oninput="GC.timeline=this.value" autocomplete="off" style="margin-top:6px">`);
...
function gcSaveCustomer(status) {
  ...
  let prop = null;
  if (GC.address) { prop = { id: uid(), label: "Main", address: GC.address, unit: (GC.unit || "").trim(), accessNotes: "", lat: null, lng: null, customerIds: [c.id], updatedAt: now() }; if (!d.properties) d.properties = []; d.properties.push(prop); if (typeof geocodeProp === "function") geocodeProp(prop); }
  save();
  return { c: c, prop: prop };
}
function gcWizFor(c, prop) {
  const me = (typeof curUser === "function") ? curUser() : null;
  return { step: "pick", cust: { id: c.id, name: c.name, phone: GC.phone || "", address: (typeof fullAddr === "function" ? fullAddr({ address: GC.address, unit: GC.unit }) : GC.address) || "", source: "Phone call", notes: (GC.need ? ("📞 Call: " + GC.need + (GC.timeline ? " · when: " + GC.timeline : "")) : ""), propertyId: prop ? prop.id : "", soldBy: me ? me.id : "" }, items: [], recurring: false, disc: 0, discPct: null, miles: 0, hours: 0, crewN: GC.solo ? 1 : 2, junkCrew: GC.solo ? 1 : 2, disposalTrip: false, haul: "pickup", zone: "local", travelMiles: null, svc: null, inp: {}, deep: {}, deepMods: {}, deepSearch: "", id: null, invoiced: false, paid: false, paymentLink: "", finalPrice: 0, adjNote: "" };
}
