...
function load(){
  ...
  ["obx","jam"].forEach(b=>{
    ...
    if(!Array.isArray(S[b].studioVideos))S[b].studioVideos=[];   // STUDIO (js/128): long-form footage awaiting an edit {id"sv_",name,size,targets[],note,status}. The FILE itself never lives in data.json — it streams to ~/studio/raw/ via /api/video/*; this is only the record of what it is and what it's for. Holds no money -> finance byte-identical.
    if(!Array.isArray(S[b].personalFiles))S[b].personalFiles=[];   // HANDOFF FILES (js/127): documents Ray uploads for me to read {id"pf_",blobId,name,size,note,ts}. He has no direct access to the workstation, so this is the door — statements/PDFs/CSVs/screenshots land as served blobs in uploads/ and the record keeps the id + what it is. Holds no money -> finance byte-identical. Additive, empty by default.
    ...
    if(!Array.isArray(S[b].shiftNotes))S[b].shiftNotes=[];   // SHIFT NOTES (js/127): timestamped notes attached to a timeclock punch {id"sn_",entryId,ts,text,userId}. Its OWN collection rather than an array on the entry, because notes are added piecemeal through a day (possibly from two devices) and a nested array would lose one to whole-record LWW — the same lesson that split job.materials into jobMaterials.
    if(!Array.isArray(S[b].reminders))S[b].reminders=[];   // REMINDERS (js/133): {id"rm_",text,dueAt,fired,sourceNoteId,userId} — a thing to be told at a time. Its OWN collection, not a flag on a to-do: a reminder FIRES once at a moment and then is history, while a to-do is a standing item you tick. The server sweep marks `fired` server-side, so it must be a record the sync layer merges per-record like any other.
    if(!Array.isArray(S[b].billRates))S[b].billRates=[];   // BILLABLE RATES (js/136): {id"br_",kind:"base"|"mod",name,value,customerId,flat,isDefault}. base = $/hr (no customer = standard, with one = that customer's contract rate); mod = a multiplier applied on top, so "Emergency at the escape room" is their $55 x 1.5 without a bespoke record. Its OWN collection because Ray edits the card over time from more than one device.
    if(!Array.isArray(S[b].workoutLogs))S[b].workoutLogs=[];   // WORKOUTS (js/139): a MIRROR of Ray's own rjworkout.html, which stays the source of truth. One record per session {id"wk_<date>_<day>",date,dayName,exercises,setCount,volume} plus one kind:"body" series record. Summarised, not the raw blob — his state copies `intents` into all 120 history days, which would bloat data.json for nothing.
    if(!Array.isArray(S[b].routineItems))S[b].routineItems=[];   // ROUTINE (js/141): the shape of a day — {id"rt_",label,part:"morning"|"day"|"evening",action,order,doneOn}. `doneOn` is a DATE STRING, not a flag: done-today means doneOn===today, so a new day resets every item with nothing running at midnight. It also means there is no completion history, which is deliberate — a daily checklist with history is one step from a streak.
    if(!Array.isArray(S[b].payLinks))S[b].payLinks=[];   // PAY LINKS (server /i/): {id"plk_"+key,key,url,linkId,amountCents} — ONE live Stripe balance link per invoice (or billed-together group), minted + refreshed by the SERVER as the balance moves. Its own server-owned collection (never a field on the quote) so refreshing a link can never clobber a quote edit.
    if(!Array.isArray(S[b].invoiceViews))S[b].invoiceViews=[];   // INVOICE OPENS (js/46 + server /i/): {id"iv…",quoteId,at,ua} — one record per CUSTOMER open of the hosted invoice page. Written by the SERVER (the page is server-rendered; the app never sees the customer's browser), read here to show "opened N× — last …". Owner preview opens are excluded server-side (?preview + the jso cookie). Its own collection: per-record LWW, server-only writer, so it can never clobber a quote.
    if(!Array.isArray(S[b].followUps))S[b].followUps=[];   // COLLECTIONS (js/154): one row per chase — {id"fu_",quoteId,customerId,sentAt,channel,text}. Its own collection, NOT an array on the quote: per-record LWW would clobber the whole log every time two devices touched that quote. It is a record of what he SENT, never a queue of what to send — nothing here reaches a customer until he taps send himself.
    ...
    if(!Array.isArray(S[b].catalogSkus))S[b].catalogSkus=[];   // PRODUCT BACK END (js/129/130): our own SKUs — {id"sku_",sku,cat,name,brand,model,specs,cost,costNote,sources[{vendor,price,url}],sell,sellNote,laborHours,tier,stock,status,confidence,notes}. `cost` is the STANDARD assumed buy price, not the last price paid, so a quote is never built on a number that moved. Holds no money of its own -> finance byte-identical. Additive, empty by default.
  ...
  (S.registry||[]).forEach(r=>{const b=r&&r.id;if(!b||!S[b]||typeof S[b]!=="object"||Array.isArray(S[b]))return;
    ...
    if(!Array.isArray(S[b].studioVideos))S[b].studioVideos=[];  // STUDIO: backfill on EVERY org slab
    if(!Array.isArray(S[b].personalFiles))S[b].personalFiles=[];  // HANDOFF FILES: backfill on EVERY org slab
    ...
    if(!Array.isArray(S[b].shiftNotes))S[b].shiftNotes=[];  // SHIFT NOTES: backfill on EVERY org slab so the sync layer / js/127 always find the array
    if(!Array.isArray(S[b].shiftNotes))S[b].shiftNotes=[];  // SHIFT NOTES: backfill on EVERY org slab so the sync layer / js/127 always find the array
    if(!Array.isArray(S[b].reminders))S[b].reminders=[];  // REMINDERS: backfill on EVERY org slab so the sync layer / js/133 always find the array
    if(!Array.isArray(S[b].billRates))S[b].billRates=[];  // BILLABLE RATES: backfill on EVERY org slab so the sync layer / js/136 always find the array
    if(!Array.isArray(S[b].workoutLogs))S[b].workoutLogs=[];  // WORKOUTS: backfill on EVERY org slab so the sync layer / js/139 always find the array
    if(!Array.isArray(S[b].routineItems))S[b].routineItems=[];  // ROUTINE: backfill on EVERY org slab so the sync layer / js/141 always find the array
    if(!Array.isArray(S[b].payLinks))S[b].payLinks=[];  // PAY LINKS: backfill on EVERY org slab so the sync layer always finds the array
    if(!Array.isArray(S[b].invoiceViews))S[b].invoiceViews=[];  // INVOICE OPENS: backfill on EVERY org slab so the sync layer / js/46 always find the array
    if(!Array.isArray(S[b].followUps))S[b].followUps=[];  // COLLECTIONS: backfill on EVERY org slab so the sync layer / js/154 always find the array
    ...
    if(!Array.isArray(S[b].catalogSkus))S[b].catalogSkus=[];  // PRODUCT BACK END: backfill on EVERY org slab so the sync layer / js/129 always find the array
