...
/* ⭐ WHITE GOODS + MATTRESSES RIDE TO MANNS HARBOR (Ray, 2026-09-17): Soundside Recycling does NOT take refrigerators or
   mattresses. Those go to Dare County's Manns Harbor transfer station ($75/ton; Currituck's Maple station is $105 in-county /
   $115 out-of-county, so Dare wins every time). That run is ~88 mi round trip / ~110 min from the shop, and junk is stashed
   and batched, so the run is shared by the white-goods units on it. Per-unit fee = the run's share + tipping (~$8 a fridge). */
const JUNK_MH_TRIP_MILES=88, JUNK_MH_TRIP_MIN=110, JUNK_MH_UNITS_PER_RUN=8;   // Manns Harbor round trip · minutes · units a run carries
const JUNK_MH_RUN_COST=Math.round(JUNK_MH_TRIP_MILES*0.725 + (JUNK_MH_TRIP_MIN/60)*(45/0.48));   // ≈ $236: mileage + one person's drive time at the loaded rate
const JUNK_MH_UNIT_FEE=Math.round((JUNK_MH_RUN_COST/JUNK_MH_UNITS_PER_RUN + 8)/5)*5;            // ≈ $35 a unit (run share + tipping), rounded to $5
const JUNK_FEE={freon:JUNK_MH_UNIT_FEE,mattress:JUNK_MH_UNIT_FEE,tire:8,ewaste:30,paint:10,appliance:25};
...
function wizJunkUI(){
  ...
  if(c.counts.freon)h+=`<div class="card" style="border-left:4px solid var(--warn,#c90);font-size:12.5px;line-height:1.5">❄️ ${c.counts.freon} Freon unit(s): Soundside does not take these. They ride to Dare County's Manns Harbor transfer station ($75/ton, the county recovers the refrigerant, no EPA tech on our side). That is an ~88-mile round trip, so the price carries $${JUNK_MH_UNIT_FEE} a unit: its share of the run plus tipping. Keep them upright with the sealed system intact.</div>`;
...
window.wizAddJunk=function(){
  ...
  if(c.counts.freon)notes.push(c.counts.freon+" Freon unit(s) — Manns Harbor transfer station run (Soundside does not take them); $"+JUNK_MH_UNIT_FEE+"/unit covers the run share + tipping.");
  if(c.counts.mattress)notes.push(c.counts.mattress+" mattress(es) — Manns Harbor transfer station run (Soundside does not take them); $"+JUNK_MH_UNIT_FEE+"/unit covers the run share + tipping.");
