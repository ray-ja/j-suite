...
function rcptParseSuggestion(text, cats, jobIds) {
  ...
  const last4 = (typeof o.last4 === "string" && /^\d{4}$/.test(o.last4)) ? o.last4 : null;   // js/94: card used, if the receipt prints it
  ...
  if (Array.isArray(o.transactions)) {
    ...
    for (const tx of o.transactions) {
      ...
      const tl4 = (typeof tx.last4 === "string" && /^\d{4}$/.test(tx.last4)) ? tx.last4 : null;
      cleanTx.push({ vendor: String(tx.vendor || "").slice(0, 120), amount: tamt, date: tdate, last4: tl4, type: ttype, category: tcat, refund: tx.refund === true });
    }
    transactions = cleanTx;
  }
  return { vendor: String(o.vendor || "").slice(0, 120), amount: amount, date: date,
    desc: String(o.desc || "").slice(0, 200), type: type, category: category, jobId: jobId, last4: last4, refund: refund, deposit: deposit, splits: splits, lineItems: lineItems, transactions: transactions, confidence: confidence };
}
