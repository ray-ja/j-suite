/* Headless load + smoke-test harness for "Business App (v1).html".
   ...
   Usage: node verify-app.js "<smoke JS>"   (smoke runs after boot; throw or push to __errs to FAIL; call diag("msg") to print a non-failing DIAG: line)
...
try{
  ...
  const diag=md?JSON.parse(md[1].replace(/&quot;/g,'"').replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">")):[];
  if(diag&&diag.length)diag.forEach(d=>console.log("DIAG: "+d));
