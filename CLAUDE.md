...
- Cost model = hard costs only. Disposal — **EVERY POUND IS BILLABLE; there is no free allowance in a quote** (the 500 lb C&D waiver is an annual RESIDENTIAL one, corrected 2026-08-26; same class of error as the veg one below). Rates = the close station Ray switched to 2026-08-28 (15 min from base, replaced the 55-mi Maple round trip; per-run math favours it at any trailer-size load): **mixed C&D $90/ton · yard waste $75/ton · dirty concrete $90/ton** (clean asphalt $35/load and clean concrete $50-min exist but we don't do those). **Fridges, freezers, ACs and mattresses** (Ray, 2026-09-18): Soundside only takes one "mixed in with a load of C&D", which we can't count on, so they go to Dare County's Manns Harbor transfer station ($75/ton, county recovers the freon), 50 min each way = two hours round trip. We STASH them at the warehouse: decent fridges get fixed and resold (Ray), the rest batch to Manns Harbor, so per unit: fridge $25, mattress $50 (a sixth of the ≈$251 run), window AC $25, tipping included (js/21 `JUNK_MH_*`, `JUNK_FEE`). Junk minimums are per crew size: one person $175, two people $300 (`JUNK_MIN_BY_CREW`); the wizard shows both prices and the call script offers both. Currituck's Maple station is $105/ton in-county, $115 out-of-county, so Dare always wins. Veg/brush is NOT free for a contractor — Dare's free residential yard site excludes contractor/lot-clearing debris. Mileage at the IRS rate $0.725/mi, materials, equipment rental. No hourly-labor cost line — the owners are paid from a revenue split, not wages, so labor isn't a per-job cost. Show Cost / Price / Profit / Margin with a 35% margin-floor warning.
